...
def swine_align(player_score, opponent_score):
    ...
    while(potential_gcd <= player_score and potential_gcd <= opponent_score):
        if(player_score % potential_gcd == 0 and opponent_score % potential_gcd == 0):
            greatest_gcd = potential_gcd
        potential_gcd += 1
    return greatest_gcd >= 10
    # END PROBLEM 4a
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    max_roll = 0
    while index <= 10:    
        max_temp = averaged_dice(index, dice)
        if max_temp > max_roll:
            max_roll = max_temp
            max_num_roll = index
        index += 1
    return max_num_roll
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    elif score >= opponent_score and score > 80:
        return bacon_strategy(score, opponent_score, cutoff=7, num_rolls=2)
    else:
        return bacon_strategy(score, opponent_score, cutoff=8, num_rolls=5)
