...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while current_place is not beehive:
            if self.min_range <= current_range <= self.max_range and current_place.bees:
                return rANTdom_else_none(current_place.bees)
            # if  current_range >= self.min_range and current_range <= self.max_range and current_place.bees:
            #    return rANTdom_else_none(current_place.bees)
    ...
    def action(self, gamestate):
        ...
        if self.is_TrueQueen:
            ...
            while current_place is not None:
                if current_place.ant is not None and not current_place.ant.is_double_damage:
                    current_place.ant.damage *= 2
                    current_place.ant.is_double_damage = True
...
def apply_status(status, bee, length):
    ...
    # BEGIN Problem Optional 4
    "*** YOUR CODE HERE ***"
    # because when the length runs out, we still need the old action
    old_action = bee.action
    new_action = status(old_action, bee)
    def new_status_action(gamestate):
        nonlocal length
        if length > 0:
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem Optional 4
        "*** YOUR CODE HERE ***"
        if target and target.ever_rev_direction == False:
            target.ever_rev_direction = True
        ...
        while current_place is not beehive:
            if current_place.bees != []:
                for bee in current_place.bees:
                    insect_and_place[bee] = distance
            if current_place.ant is not None and current_place.ant is not self:
                insect_and_place[current_place.ant] = distance
            distance += 1
            current_place = current_place.entrance
        return insect_and_place
        # END Problem Optional 5
