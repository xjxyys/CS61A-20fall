...
def about(topic):
    ...
    def select(paragraph):
        clean_paragraph = split(lower(remove_punctuation(paragraph)))
        for word in topic:
            if word in clean_paragraph:
                return True
        return False
    return select
    # END PROBLEM 2
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        ...
        diff_sequ = [diff_function(user_word, word, limit) for word in valid_words]
...
def time_per_word(times_per_player, words):
    ...
    for list in times_per_player:
        ...
        for i in range(1, len(list)):
            tmp += [list[i] - list[i-1]]
        real_time += [tmp] 
#    game = [words, real_time]
    return game(words, real_time)
    # END PROBLEM 9
...
def fastest_words(game):
    ...
    for j in word_indices:
        ...
        for i in player_indices:
            player_word_time = time(game, i, j)
            if player_word_time < min_time:
                min_time = player_word_time
                min_player = i
        fastest_words_lists[min_player] += [word_at(game, j)]
