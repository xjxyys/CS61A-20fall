...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        expr_operator = scheme_eval(first, env) # scheme_apply contains validate_procedure
        if isinstance(expr_operator, MacroProcedure):
            return scheme_eval(expr_operator.apply_macro(rest, env), env)
        return scheme_apply(expr_operator, rest.map(lambda x: scheme_eval(x, env)), env) # scheme_apply(procedure, args, env)
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        formals = target.rest # Pair('x', nil) (x)
        body = expressions.rest # Pair(Pair('+', Pair('x', Pair(2, nil))), nil) ((+ x 2))
        lambda_expression = Pair(formals, body) # Pair(Pair('x', nil), Pair(Pair('+', Pair('x', Pair(2, nil))), nil))
        value = do_lambda_form(lambda_expression, env) # LambdaProcedure(Pair('x', nil), Pair(Pair('+', Pair('x', Pair(2, nil))), nil), <Global Frame>)
...
def do_if_form(expressions, env):
    ...
    if is_true_primitive(scheme_eval(expressions.first, env)): # 这一行不用传入true
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        names = Pair(pair.first, names) # Pair('x', Pair('y', nil))
        values = Pair(scheme_eval(pair.rest.first, env), values) # Pair(2, Pair(3, nil))
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('bad argument to define-macro')
    # END Problem 20
