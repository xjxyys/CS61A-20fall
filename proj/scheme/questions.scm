...
(define (zip pairs)
  (cond ((null? pairs) (cons nil (cons nil nil)))
        (else (cons (cons (car (car pairs)) (car (zip (cdr pairs))))
              (cons (cons (car (cdr (car pairs))) (car (cdr (zip (cdr pairs)))))  nil)
              ))
  )
)
...
(define (enumerate s)
  ; BEGIN PROBLEM 15
  (define (enumerate_from start s)
    (if (null? s)
      nil
      (cons (list start (car s)) (enumerate_from (+ start 1) (cdr s)))))
  (enumerate_from 0 s)
  )
  ; END PROBLEM 15
  ...
  (cond ((atom? expr)
        ...
        ((let? expr)
         (let ((values (cadr expr))
           ...
           (define params (map let-to-lambda (car (cdr (zip values)))))
