...
def preorder(t):
    ...
    if is_leaf(t):
        ...
        result = [label(t)]
        result.extend(sum([preorder(b) for b in branches(t)], []))
        return result
...
def multiple_references_explanation():
    return """The multiple reference problem is the consequnce of repeatedly using number 
    that is uncertain. To solve this problem, we should reduce the use of the uncertain 
    number, best, we use it only once"""
...
def quadratic(x, a, b, c):
    ...
    if x1 <= s and x2 >= s:
        if a > 0:
            return interval(top, max(f1, f2))
        else:
            return interval(min(f1, f2), top)
    else:
