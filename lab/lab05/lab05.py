...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    oriented_city = make_city('oriented_city', lat, lon)
    return get_name(min(city_a, city_b, key = lambda city: distance(oriented_city, city)))
...
#    result_branch = []
#    for b1, b2 in zip(branches(b1), branches(b2)):
...
#        result_branch += [new_branch]
    ...
    k = len(result_branch)
    result_branch += branches(t1)[k:]
    result_branch += branches(t2)[k:] #one of the branches(ti)[k:] is empty list
