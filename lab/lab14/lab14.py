...
def prune_min(t):
    ...
    if t.is_leaf():
        ...
        smaller_branches = [b for b in t.branches if b.label == min([b.label for b in t.branches])]
        prune_min(smaller_branches[0])
        t.branches = smaller_branches
...
def num_splits(s, d):
    ...
    "*** YOUR CODE HERE ***"
    assert len(s) > 1
    # 由于这是求和，我们只需要记住一个列表即可，被切分的列表的和等于总和减去这个第一列表的和
    def helper_split(num_list, d, s, i=0):
        # i是当前的索引
        if i == len(s):
            # i = len(s)说明要完成最后一次加法
            remaining = [x for x in s if x not in num_list]
            return 1 if abs(sum(num_list) - sum(remaining)) <= d else 0
        else:
            choose_1 = helper_split(num_list + [s[i]], d, s, i + 1)
            choose_2 = helper_split(num_list, d, s, i + 1)
            return choose_1 + choose_2
        
    return helper_split([], d, s, 0) // 2
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            skeleton_match, skeleton_cost = helper_align(skeleton_idx + 1, code_idx + 1)
            skeleton_match = skel_char + skeleton_match
            possibilities.append((skeleton_match, skeleton_cost))
        # Insert
        code_insert, code_cost = helper_align(skeleton_idx, code_idx + 1)
        code_insert = "+[" + code_char + "]" + code_insert
        possibilities.append((code_insert, code_cost + 1))
        # Delete
        skeleton_delete, skeleton_cost = helper_align(skeleton_idx + 1, code_idx)
        skeleton_delete = "-[" + skel_char + "]" + skeleton_delete
        possibilities.append((skeleton_delete, skeleton_cost + 1))
...
def filterl(lst, pred):
    ...
    return foldr(lst, helper_fn, Link.empty)
...
def foldl2(link, fn, z):
    ...
    # (step 3, (step 2, (step 1, identity))) (z)
    # (f(a, 1))
    # f(f(a, 2), 1)
    # f(f(f(a, 3), 2), 1)
    # foldl : f(f(f(a, 3), 2), 1)
